...
5. **Verify citations by spot-checking.** You can read the codebase (with `read`/`ffgrep`/`bash`) and call MCP tools through the pi `mcp` gateway to verify citations; do not write files, commit, or modify external state. If you're uncertain a cited item exists or says what's claimed, check it. Don't propagate errors.
