...
Start by finding the relevant code. Use `fffind` to locate directories and files, `ffgrep` to find key symbols, `read` to understand the actual implementation. Don't guess from names. Read the code. You have read-only access only — never write, edit, or commit.
