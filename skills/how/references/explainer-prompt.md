...
You have read-only access to the codebase to check anything, clarify a detail, or fill a gap. Use `read`, `ffgrep`, and `fffind` as needed. The explorers did the heavy lifting, so you shouldn't need to re-explore from scratch. Don't write, edit, or commit anything.
