...
- `references/workflow-sdk-building.md` — Workflow creation, settings, JSON import/export, and complete building patterns
- `references/workflow-sdk-nodes-connections.md` — All node factories, subnodes (AI/LangChain), connection patterns, and credentials
- `references/workflow-sdk-control-flow.md` — IF/Else, Switch/Case, Split in Batches, Merge, error handling, and branching
- `references/workflow-sdk-expressions-code.md` — Expression system, Code node helpers, fromAi, and expression context
- `references/workflow-sdk-validation-testing.md` — Workflow validation, pin data, test data generation, and testing strategies
- `references/workflow-sdk-code-generation.md` — JSON-to-code, code-to-JSON, parseWorkflowCode, and the generation pipeline
- `references/workflow-sdk-plugins.md` — Plugin system, type generation, Zod schemas, and extensibility
...
- `references/node-registry-properties.jsonl` — Node properties/parameters (one JSON line per node, grep for a specific node). **Not shipped by default** — prefer `n8n_mcp_get_node_types`, or generate with `scripts/refresh-node-registry.sh`.
...
**Always look up the real node type** before using it. **Prefer the live MCP** when one is
connected: `n8n_mcp_search_nodes` to find the node, `n8n_mcp_get_node_types` for its exact
parameters/version (always current). The offline registry cache below is the fallback when
no MCP is available. See [mcp-workflow-tools.md](mcp-workflow-tools.md).

### Local Registry Cache (offline fallback)

This skill ships small index caches for offline node lookup. The large properties cache is
**not shipped** — generate it on demand with `bash scripts/refresh-node-registry.sh`.
...
- **`references/node-registry-properties.jsonl`** — Full node properties/parameters (one JSON line per node). **NOT shipped**; run the refresh script to generate it, or use `n8n_mcp_get_node_types` instead.
...
6. **Grep `references/node-registry-properties.jsonl`** (or call `n8n_mcp_get_node_types`) for the node name to get its available parameters and configure it correctly
...
|----------|---------|-----------|
| `workflow(id, name, options?)` | Create workflow builder | `references/workflow-sdk-building.md` |
| `workflow.fromJSON(json)` | Import from n8n JSON | `references/workflow-sdk-building.md` |
| `node(input)` | Create a regular node | `references/workflow-sdk-nodes-connections.md` |
| `trigger(input)` | Create a trigger node | `references/workflow-sdk-nodes-connections.md` |
| `sticky(content, nodes?, config?)` | Create a sticky note | `references/workflow-sdk-nodes-connections.md` |
| `placeholder(hint)` | Create a placeholder value | `references/workflow-sdk-nodes-connections.md` |
| `newCredential(name)` | Mark credential as new | `references/workflow-sdk-nodes-connections.md` |
| `ifElse(config)` | Create IF node | `references/workflow-sdk-control-flow.md` |
| `switchCase(config)` | Create Switch node | `references/workflow-sdk-control-flow.md` |
| `merge(config)` | Create Merge node | `references/workflow-sdk-control-flow.md` |
| `splitInBatches(config)` | Create Split In Batches | `references/workflow-sdk-control-flow.md` |
| `nextBatch(sib)` | Loop-back for SIB | `references/workflow-sdk-control-flow.md` |
...
See `references/workflow-sdk-validation-testing.md` for the complete list and resolution strategies.
