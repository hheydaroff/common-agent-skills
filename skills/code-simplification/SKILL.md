...
description: "Simplify code for clarity without changing behavior. Use when refactoring for readability, when code works but is harder than it should be to read/maintain/extend, when reviewing code that accumulated unnecessary complexity, or when user says 'simplify this', 'clean this up', 'make this readable'."
---
...
```
Before simplifying:
1. Read project conventions / rules files
2. Study how neighboring code handles similar patterns
3. Match the project's style for imports, declarations, naming, errors, types
```
...
| Nested ternaries | Requires mental stack | Replace with if/else or lookup |
| Boolean parameter flags | `doThing(true, false, true)` | Options objects or separate functions |
| Repeated conditionals | Same `if` check everywhere | Extract to a named predicate |
...
| Abbreviated names | `usr`, `cfg`, `btn`, `evt` | Use full words (unless universal: `id`, `url`) |
...
| Comments explaining "why" | `// Retry because API is flaky` | Keep — carries intent |
...
| Unnecessary abstractions | Wrapper adding no value | Inline |
...
**The Rule of 500:** If a refactoring touches more than 500 lines, invest in automation (codemods, sed, AST transforms) rather than hand-editing.
...
| "The original author must have had a reason" | Check git blame — apply Chesterton's Fence. But sometimes it's just iteration residue |
...
- [ ] The diff is clean — no unrelated changes
- [ ] Simplified code follows project conventions
- [ ] No error handling was removed or weakened
- [ ] No dead code left behind
- [ ] A teammate or review agent would approve the change as net improvement
